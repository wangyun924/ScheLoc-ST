...
void readParameter(IloInt num1, IloInt num2, IloInt* Nbj, IloInt* Nbl, IloInt* Nbm)
{
	...
	_snprintf_s(NEWinstance, sizeof(NEWinstance), "C:\\Users\\DELL\\OneDrive\\Programming\\ScheLoc-ST\\Programme\\ScheLoc-setup-times\\data\\data-SingleObj-MILP-LBBD\\Small\\Example%d_%d.dat", num1, num2);
...
int main() {
	srand(time(NULL));
	int timeLim = 1800;
	ofstream out("C:\\Users\\DELL\\OneDrive\\Programming\\ScheLoc-ST\\Programme\\ScheLoc-setup-times\\record\\Two-indexed-SingleObj\\Model\\Small-EJOR\\ScheLoc-SS-MILP-Small-summarize.txt", ios::app);
	for (num1 = 1; num1 <= 1; num1 += 1)
		for (num2 = 0; num2 <= 0; num2++) // Run each instance
		{
			IloNum timeCPU = 0;
			IloEnv env;
			IloTimer timer(env);
			...
			try
			{
				...
				char instance[200];
				sprintf_s(instance, "C:\\Users\\DELL\\OneDrive\\Programming\\ScheLoc-ST\\Programme\\ScheLoc-setup-times\\record\\Two-indexed-SingleObj\\Model\\Small-EJOR\\Example%d_%d.dat", num1, num2);
				...
				out << " best UB = : " << mycplex.getObjValue() << "   best LB: " << mycplex.getBestObjValue() << "\t";
